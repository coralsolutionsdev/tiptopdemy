<script>
    ...
    function resetMediaModal(resetStatus = false) {
        ...
        $("select[name=media_type]").val({{\App\Modules\Media\MediaFile::TYPE_YOUTUBE}})
    ...
    function drawMediaItem(media) {
        ...
        $.post('{{route('store.add.resources.item', $lesson->slug)}}', media).done(function (response) {
            ...
            var id = media.id;
            var video = null;
            if (status == {{\App\Modules\Media\MediaFile::UPLOAD_TYPE_COMPLETED}}) {
                // insert media
                if (type == {{\App\Modules\Media\MediaFile::TYPE_VIDEO}}) {
                    video = '<video src="' + url + '" muted playsinline controls disablepictureinpicture controlsList="nodownload"></video>';
                } else if (type == {{\App\Modules\Media\MediaFile::TYPE_YOUTUBE}} || type == {{\App\Modules\Media\MediaFile::TYPE_HTML_PAGE}}) {
                ...
                deleteResourceItem();
                UIkit.notification("<span uk-icon='icon: check'></span> " + response.message, {
                    pos: 'top-center',
                    status: 'success'
                })
                ...
                resetMediaModal();
            } else if (status == {{\App\Modules\Media\MediaFile::UPLOAD_TYPE_REFUSED}}) {
                // error
                UIkit.notification("<span uk-icon='icon: warning'></span> Oops! something went wrong!", {
                    pos: 'top-center',
                    status: 'danger'
                });
    ...
    $('#resourceStartUpload').click(function () {
        ...
        if (path == '') {
            UIkit.notification("<span uk-icon='icon: minus-circle'></span> Please add an embed link first", {
                pos: 'top-center',
                status: 'danger'
            });
        ...
        init: function () {
            ...
            submitBtn.click(function () {
                if (myDropZone.files && myDropZone.files.length > 0) {
                    ...
                    UIkit.notification("<span uk-icon='icon: warning'></span> Please select a file to upload first.", {
                        pos: 'top-center',
                        status: 'warning'
                    })
                }
            ...
            this.on("error", function (event) {
                console.log(event);
                UIkit.notification("<span uk-icon='icon: warning'></span> " + event.status, {
                    pos: 'top-center',
                    status: 'danger'
                });
                submitBtn.html("{{__('main.Start upload')}}");
