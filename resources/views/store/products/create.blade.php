...
@section('page-header-button')
    <button class="btn btn-primary btn-lg w-75">
        <span>{{!empty($product)? __('main.Save changes') : __('main.submit')}}</span></button>
@endsection
@section('head')
...
@section('content')
        ...
        <div class="form-panel row">
            <div class="col-lg-12">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link product-info-link active" id="product-info-tab" data-toggle="tab"
                           href="#product-info" role="tab" aria-controls="product-info"
                           aria-selected="true">{{__('main.Product info')}}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link images-link" id="images-tab" data-toggle="tab" href="#images" role="tab"
                           aria-controls="images" aria-selected="false">{{__('main.Images')}}</a>
                    </li>
                    @if(!empty($product))
                        <li class="nav-item">
                            <a class="nav-link attributes-link" id="attributes-tab" data-toggle="tab" href="#attributes"
                               role="tab" aria-controls="attributes" aria-selected="false">{{__('main.Attributes')}}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link lessons-link" id="lessons-tab" data-toggle="tab" href="#lessons"
                               role="tab" aria-controls="lessons" aria-selected="false">{{__('main.Lessons')}}</a>
                        </li>
                    @endif

                </ul>
                <div class="tab-content" id="myTabContent">
                    <div class="tab-pane fade show product-info-tab active" id="product-info" role="tabpanel"
                        ...
                        <div class="card border-light">
                            <div class="card-body">
                                ...
                                <div class="form-group row col-lg-12">
                                    ...
                                    <div class="col-lg-10 padding-0 margin-0">
                                        <div>
                                            <span class="btn btn-primary" data-toggle="collapse"
                                                  data-target="#demo">{{__('main.Show list of categories')}}</span>
                                ...
                                <div class="form-group row col-lg-12">
                                    ...
                                    <div class="col-lg-10 padding-0 margin-0">
                                        <input type="checkbox" name="manage_stock" class="toogle-switch manage_stock"
                                               value="1" {{empty($product) || !empty($product->manage_stock) ? 'checked' : null}}>
                                    </div>
                                </div>
                                <div class="form-group row col-lg-12">
                                        ...
                                        <div class="row">
                                                    ...
                                                    <select name="scope_id" class="form-control scope">
                                                        @foreach(getInstitutionScopes() as $scope)
                                                            <option value="{{$scope->id}}"
                                                                    {{(!empty($product) && $product->scope_id == $scope->id) || (empty($product) && $scope->default == 1)  ? 'selected' : ''}} title="{{$scope->description}}">{{$scope->title}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-lg-3">
                                                <div class="form-group fields-items-section">
                                                    <select name="field_option_id"
                                                            class="form-control field-item-options">
                                                ...
                                                <div class="col-3">
                                                    <div class="form-check" style="margin: 5px">
                                                        <input class="form-check-input" type="radio"
                                                               ...
                                                               id="color-pattern-{{$colorPattern->id}}"
                                                               value="{{$colorPattern->id}}" {{(!empty($product) && $product->color_pattern_id == $colorPattern->id) || $colorPattern->default == 1 ? 'checked' : ''}}>
                                                        <label class="form-check-label"
                                                               for="color-pattern-{{$colorPattern->id}}"
                                                               style="display: block">
                                                            <div class="text-center"
                                                                 style="background: linear-gradient({{$colorPattern->angle}}deg,{{str_replace(['"', '[', ']'], '', json_encode($colorPattern->gradient))}}); padding: 15px 20px; color: white; cursor: pointer; border-radius: 5px">
                                                                {{$colorPattern->title}}
                                                            </div>
                                                        </label>
                                                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group row col-lg-12">
                                    ...
                                    <div class="col-lg-10 padding-0 margin-0">
                                        ...
                                        <div>
                                            <ul class="uk-grid-small uk-child-width-1-2 uk-child-width-1-2@s uk-child-width-1-3@m media-items-list"
                                                uk-sortable="handle: .uk-sortable-handle" uk-grid="masonry: true">

                                                @if(!empty($product) && $productMedia = $product->getMedia(\App\Modules\Media\MediaFile::getGroup(\App\Modules\Media\MediaFile::TYPE_PRODUCT_IMAGE)))
                                                    @foreach($productMedia as $image)
                                                        <li id="media_item-{{$image->id}}">
                                                            <div class="uk-card uk-card-default uk-card-body uk-padding-remove">
                                                                <div class="bg-white uk-box-shadow-hover-medium resource-item-control">
                                                                    <span class="uk-sortable-handle uk-margin-small-right hover-primary"
                                                                          uk-icon="icon: table"></span> <span
                                                                            uk-tooltip="{{__('main.delete')}}"
                                                                            class="hover-danger media-delete"
                                                                            uk-icon="icon: trash"></span></div>
                                                                <div>
                                                                    <input type="hidden" name="media_id[]"
                                                                           value="{{$image->id}}">
                                                                    <input type="hidden" name="media_position[]"
                                                                           value="0">
                                                                    <input type="hidden" name="media_new_file_order[]"
                                                                           value="">
                                                                    <img data-src="{{$image->getFullUrl('card')}}"
                                                                         sizes="(min-width: 650px) 650px, 100vw"
                                                                         width="650" height="433" alt="" uk-img>
                                                                </div>
                                                            </div>
                                                        </li>
                                                    @endforeach
                    ...
                    @if(!empty($product))
                        <div class="tab-pane fade attributes-tab" id="attributes" role="tabpanel"
                                                    ...
                                                    @foreach ($attribute->options as $option)
                                                        <div class="form-group d-flex align-items-center">
                                                            <label class="radio-inline"><input type="radio"
                                                                                               name="{{ $attribute->id }}"
                                                                                               value="{{ $option->value }}" {{$product->getProductAttrOptionValueById($attribute->id, $option->id) == $option->value ? 'checked' : '' }} />&nbsp
                                                                &nbsp{{ $option->name }}</label>&nbsp;&nbsp;
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                                                    ...
                                                    @foreach ($attribute->options as $option)
                                                        <div class="form-group d-flex align-items-center">
                                                            <input type="checkbox" name="{{ $attribute->id }}[]"
                                                                   class="toogle-switch manage_stock"
                                                                   value="{{ $option->value }}" {!! $product->hasAttributeValue($attribute->name, $option->value) ? 'checked' : ''  !!}>&nbsp
                                                            &nbsp{{ $option->name }}
                                                        </div>
                                                    @endforeach
                                                </div>
                        ...
                        <div class="tab-pane fade lessons-tab" id="lessons" role="tabpanel"
                                    ...
                                    <div style="position: fixed; right: 30px; bottom: 30px">
                                        <div class="uk-inline">
                                            <button class="uk-button uk-button-primary" type="button"
                                                    style="border-radius: 50%; padding: 11px 20px"><span
                                                        uk-icon="icon: plus"></span></button>
                                            <div uk-dropdown style="padding: 10px 10px 0px 10px">
                                                <div><a href="{{route('store.groups.create', $product->slug)}}"
                                                        class="uk-button uk-button-default uk-button-small uk-width-1-1 uk-margin-small">{{__('main.Add new Unit')}}</a>
                                                </div>
                                                <div><a href="{{route('store.lessons.create', $product->slug)}}"
                                                        class="uk-button uk-button-default uk-button-small uk-width-1-1 uk-margin-small">{{__('main.Add new Lesson')}}</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <li class="unit {{$id == 0 ? 'uk-open' : ''}}">
                                                    <a class="uk-accordion-title" href="#">
                                                        <span class="text-primary">{{sprintf('%02d', $id+1)}} <span
                                                                ...
                                                                style="padding: 0 5px">{{$group->title}}</span>
                                                    </a>
                                                        ...
                                                        <div class="uk-grid-collapse uk-child-width-1-1" uk-grid>
                                                            ...
                                                            <div class="uk-width-auto uk-flex uk-flex-right">
                                                                <div class="action_btn text-right">
                                                                    <ul>
                                                                        <li class="">
                                                                            <a href="{{route('store.groups.edit', [$product->slug, $group->slug])}}"
                                                                               class="uk-button uk-button-small uk-action-btn uk-button-default ck-button-primary"
                                                                               uk-tooltip="{{__('main.edit')}}"><i
                                                                                        class="far fa-edit"></i>
                                                                                Edit</a>
                                                                        </li>
                                                                        <li class="">
                                                                            <span id="{{$group->id}}"
                                                                                  class="uk-button uk-button-small uk-action-btn uk-button-default ck-button-danger btn-delete"
                                                                                  uk-tooltip="{{__('main.delete')}}"><i
                                                                                        class="far fa-trash-alt"></i> Delete</span>
                                                                            <form id="delete-form" method="post"
                                                                                  action="{{route('store.groups.destroy', [$product->slug, $group->slug])}}">
                                                                                {{csrf_field()}}
                                                                                {{method_field('DELETE')}}
                                                                            </form>
                                                                        </li>
                                                            ...
                                                            <div>
                                                                <table class="uk-table uk-table-justify uk-table-divider">
                                                                    ...
                                                                    @forelse($group->getLessons as $itemId => $item)
                                                                        <tr>
                                                                            ...
                                                                            <td class="uk-table-expand align-middle">
                                                                                {{$item->title}}
                                                                                <br> <span
                                                                                        class="text-muted">{{$item->getType()}}</span>
                                                                            </td>
                                                                            ...
                                                                            <td class="uk-width-small">
                                                                                <div class="action_btn text-right"
                                                                                    ...
                                                                                    <ul>
                                                                                        ...
                                                                                        <li>
                                                                                            <a href="{{route('store.get.form.templates', $item->slug)}}"
                                                                                               class="btn btn-light">
                                                                                                {{__('main.Add Quiz')}}
                                                                                            </a>
                                                                                        </li>
                                                                                        ...
                                                                                        <li class="">
                                                                                            <a href="{{route('store.lessons.edit', [$product->slug, $item->slug])}}"
                                                                                               class="btn btn-light"><i
                                                                                        ...
                                                                                        <li class="">
                                                                                            ...
                                                                                            <form id="delete-form"
                                                                                                  method="post"
                                                                                                  action="{{route('store.lessons.destroy', [$product->slug, $item->slug])}}">
                                                                                                {{csrf_field()}}
                                                                                                {{method_field('DELETE')}}
                                                                                            </form>
                                                                                        </li>
                                                                    ...
                                                                    @empty
                                                                        <tr>
                                                                            <td colspan="4"
                                                                                class="uk-text-center">{{__('main.There is no form items yet.')}}</td>
                                                                        </tr>
                                                                    @endforelse
                                                                    </tbody>
                                                                </table>
    ...
    {{--Templaets--}}
    <section>
        <div class="product-image product-image-template"
             style="border: 1px solid var(--theme-primary-color); margin-top: 10px; display: none">
            <div class="row col-lg-12 margin-0" style="padding: 10px 0px">
                ...
                <div class="col-lg-7 d-flex align-items-center">
                    <input type="text" class="form-control" name="new_image_description[]"
                           placeholder="Image description (Optional)">
                </div>
                <div class="col-lg-2 d-flex align-items-center">
                    <input type="number" class="form-control" name="new_image_position[]" placeholder="Position"
                           value="0">
                </div>
